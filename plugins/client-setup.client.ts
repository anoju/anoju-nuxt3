// plugins/client-setup.client.ts
export default defineNuxtPlugin(() => {
  // 이 파일은 .client.ts 확장자로 인해 클라이언트에서만 실행됩니다
  ...
  const mobileClassChk = () => {
    ...
    if (isAny) {
      ...
      if (isNaverApp) rtnClass.push('naver_app');
      if (isDaumApp) rtnClass.push('daum_app');
      if (isKakaoTalk) rtnClass.push('kakao_talk');
    }
  ...
  // 플러그인 정리 시 이벤트 제거 (이 부분은 Nuxt 플러그인에서 자동으로 처리됨)
});
